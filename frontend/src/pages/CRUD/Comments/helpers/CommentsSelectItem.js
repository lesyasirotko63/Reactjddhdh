...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/comments/autocomplete`, { params });
  return response.data;
}

const CommentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.comments.hasPermissionToCreate,
});

export default connect(select)(CommentsSelectItem);
