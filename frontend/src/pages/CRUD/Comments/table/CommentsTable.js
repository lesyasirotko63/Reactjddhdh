...
import * as dataFormat from 'pages/CRUD/Comments/table/CommentsDataFormatters';

// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';
// eslint-disable-next-line
import * as articlesDataFormat from 'pages/CRUD/Articles/table/ArticlesDataFormatters';

import actions from 'actions/comments/commentsListActions';
...
const CommentsTable = ({ classes }) => {
  ...
  const count = useSelector((store) => store.comments.list.count);
  const modalOpen = useSelector((store) => store.comments.list.modalOpen);
  const rows = useSelector((store) => store.comments.list.rows);
  const idToDelete = useSelector((store) => store.comments.list.idToDelete);
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) =>
        articlesDataFormat.listFormatter(
          params.row[params.field],
          history,
          'articles',
        ),
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/comments/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Comments</h4>} disableWidgetMenu>
